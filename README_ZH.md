...
- 错误率滑动窗口追踪，高错误渠道自动降权
- 内置 API 密钥管理，支持配额、速率限制

**Prompt Engine**
- System Prompt 注入：自动在每个请求前置或追加系统消息
- 上下文压缩：限制对话历史长度，保留所有 system 消息 + 最近 N 轮对话
...
**管理面板**
- 自带 Web 管理面板，浏览器里管渠道、看日志、跑检测
- 仪表盘实时展示请求量、成功率、延迟、检测状态
- Webhook 通知（Telegram / Discord / HTTP），检测异常自动告警
- 请求缓存，可配置 TTL 和最大条目数
- 账户中心：管理上游平台账号（登录、签到、余额查询）
- Provider 预设：快速添加渠道（OpenAI、Anthropic、DeepSeek、Moonshot 等）
- 支持中英双语切换和深色模式
- 一键检查更新 + 在线升级
